...
export const getEjercicio = async (req: Request, res: Response) => {
	try {
		const id = req.params.entrenamiento as string;
...
export const createEjercicio = async (req: Request, res: Response) => {
	try {
		const plantilla = req.body.plantilla as string;
		const entrenamiento = req.body.entrenamiento as string;
...
export const updateEjercicio = async (req: Request, res: Response) => {
	try {
		...
		return res.status(200).json((await ejercicioService.updateEjercicio(id, {finalizado}))[0]);
	} catch (error) {
		return res.status(500).json({message: error});
	}
};
...
export const deleteEjercicio = async (req: Request, res: Response) => {
	try {
		const id = req.params.id as string;
		if (!id) return res.status(400).json({message: "id is required"});
		return res.status(200).json((await ejercicioService.deleteEjercicio(id))[0]);
	} catch (error) {
		return res.status(500).json({message: error});
	}
};
